...
class RequestHandler(socketserver.StreamRequestHandler):
    ...
    def finish(self):
        if self.nickname:
            ...
            if hasattr(self, 'partingWords'):
                message = "%s has quit" % (self.nickname, self.partingWords)
    ...
    def nickCommand(self, nickname):
        ...
        if self.server.users.get(nickname, None):
            raise ClientError("there is already a user name %s" % nickname)
        oldNickname = None
        if self.nickname:
    ...
    def _parseCommand(self, input):
        ...
        if input and input[0] == '/':
            ...
            commandAndArgument = input[1:].split(' ', 1)
            if len(commandAndArgument) == 2:
                command, arg = commandAndArgument
            else:
                command = commandAndArgument
